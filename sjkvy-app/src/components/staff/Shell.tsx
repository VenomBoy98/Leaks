"use client";
// Staff portal shell — same handoff design language as the applicant shell (Sal-Forest-Green
// sidebar, ivory content) with REAL routing and the signed-in staff member's identity. Wraps
// every functional staff screen. Nav mirrors the staff routes in nav-map.ts.
...
const NAV = [
  ...
  { href: "/verification", label: "Verification", icon: "fact_check" },
  { href: "/counselling", label: "Counselling", icon: "forum" },
  { href: "/staff-attendance", label: "Attendance", icon: "how_to_reg" },
  { href: "/staff-certificates", label: "Certificates", icon: "workspace_premium" },
  { href: "/staff-hostel", label: "Hostel", icon: "night_shelter" },
  { href: "/staff-placement", label: "Placement", icon: "work" },
  { href: "/staff-reports", label: "Reports", icon: "insights" },
];
    ...
    <div className="min-h-screen bg-background text-on-background">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-sidebar-width flex-col bg-primary text-on-primary md:flex">
        <div className="px-6 py-6">
          <Link href="/staff-dashboard" className="font-display-md text-display-md font-bold text-on-primary">SJKVY</Link>
          <p className="font-caption text-on-primary/70">Staff Portal</p>
        </div>
      ...
      <div className="md:pl-sidebar-width">
        <header className="flex h-topbar-height items-center justify-between border-b border-secondary/10 bg-surface/70 px-margin-mobile backdrop-blur-md md:px-margin-desktop">
          ...
          <div className="text-right">
            ...
            <p className="font-caption text-on-surface-variant">Centre staff</p>
          </div>
        </header>
